function [zgrid,xgrid,ygrid] = gridfit_weighted(x,y,z,xnodes,ynodes,w,varargin)
...
%  ynodes = 0:.1:1;
%
%  g = gridfit_weighted(x,y,z,xnodes,ynodes,w);
%
  ...
  switch params.solver
    ...
    case 'normal'
      ...
      if params.maskflag
        ...
        % Permute for minimum fill-in for \ (in chol)
        Ws = sparse(1:length(w),1:length(w),w);
%         Ws = sparse(diag(w));
        APA = A'*Ws*A;
        p = symamd(APA);
        zgrid=zeros(ny,nx);
        zgrid(p) = APA(p,p)\(A(:,p)'*Ws*rhs);
      end
