...
function [outputStreakFlow, outputSegMasks, outputOpticalFlow, outputStreakLines_x, outputStreakLines_y] = streakline_segmentation_v1 (T)
...
%     img3 = imread([pth sprintf(['/%s' img_fileformat],prefix,i+1)]);
    ...
    % save(sprintf('flow_%05d.mat',i),'hedges','u_new','v_new','xflowmap','yflowmap');
    ...
    %imwrite(optical_flow_im,sprintf(['streakflow_' img_outputfileformat],i));
    outputStreakFlow (:,:,:,i) = flow; 
    clear flow; 
    ...
    %imwrite(new_seg_mask,sprintf(['' img_outputfileformat],i));
    outputSegMasks (:,:,:,i) = new_seg_mask; 
    ...
    %imwrite(optical_flow_im,sprintf(['opticalflow_' img_outputfileformat],i));
    outputOpticalFlow (:,:,:,i) = flow;
    clear flow; 
    ...
    %axis image
    %axis ij               
    outputStreakLines_x{i,1} = xfmat_toshow; 
    outputStreakLines_y{i,1} = yfmat_toshow; 
...
%     set(gcf,'PaperPositionMode','auto')
       ...
       % print(gcf,'-djpeg','-noui',sprintf(['' img_outputfileformat],i))
